...
class Captcha {
    ...
    public function validate($code, $id = '') {
        // 验证码不能为空
        $v_code = $this->CI->session->userdata('v_code');
        $v_time = $this->CI->session->userdata('v_time');
        if(empty($code) || empty($v_code)) {
            return false;
        }
        // session 过期
        if(NOW_TIME - $v_time > $this->expire) {
            $this->CI->session->unset_userdata('v_code');
            $this->CI->session->unset_userdata('v_time');
            return false;
        }

        if( !strcasecmp($code,$v_code)) {
    ...
    public function generate($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = config_item('fonts_path');
        ...
        $this->CI->session->set_userdata('v_code', $code);
        $this->CI->session->set_userdata('v_time', NOW_TIME);
    ...
    private function _background() {
        $path = config_item('fonts_path').'/bgs/';
        $dir = dir($path);
